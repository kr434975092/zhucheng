...
<div id="body">
    ...
    <%
        List<News>list = (List<News>)session.getAttribute("news_list");
        int len = 0;
        if(list==null)len = 0;
        else len = list.size();
    %>
    <div id="container">
                	<div class="sidebar">
            	<a class="btn_create" href="Createnews.jsp">发布新闻</a>
             
<dl class="company_center_aside">
		<dt><a href="news.jsp">我发布的新闻</a></dt>
			<dd>&nbsp;</dd>
	</dl></div><!-- end .sidebar -->
            <div class="content">
            	<dl class="company_center_content">
                    <dt>
                        <h1>
                            <em></em>
                           新闻列表 <span>（共<i style="color:#fff;font-style:normal" id="positionNumber"><%=len %></i>个）</span>                        </h1>
                    </dt>
                    <dd>
                    		                    	<form id="searchForm">
			                	...
			                	<ul class="reset my_jobs">
			                	        ...
			                	        %>
				                			                            	<li>
		                                    <h3>
		                                        <a title="新闻标题" href="Job3.jsp?newsid=<%=list.get(i).getId()%>"><%=list.get(i).getTitle() %></a> 
		                                        <span>[<%=list.get(i).getCompany()%>]</span>
		                                        						                        		                                    </h3>
		                                    		                                  		
		                                  		                                    				                                    <div class="c9">发布时间<%=list.get(i).getTime() %></div>
			                                    		                                    		                                    		                                    <div class="links">
		                                    			                                       
<%-- 		                                       			                                       	<a target="_blank" class="job_edit" href="UpdateJob.jsp?jobid=<%=list.get(i).getId()%>&company=<%=list.get(i).getCompany()%>">编辑</a> --%>
		                                                             
		                                        <a class="job_del" href="Servlet/NewsServlet?method=deleteNews&newsid=<%=list.get(i).getId() %>">删除</a>
		                                    </div>
		                                    		                                </li><%} }%>
	                                	                           	</ul>
			                    			                </form>
